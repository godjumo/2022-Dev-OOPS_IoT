...
static void draw_face_boxes(fb_data_t *fb, std::list<dl::detect::result_t> *results, int face_id)
{
    ...
    if(fb->bytes_per_pixel == 2){
        //color = ((color >> 8) & 0xF0) | ((color >> 3) & 0x07E0) | (color & 0x001F);
...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    if (!detection_enabled || fb->width > 400)
    {
        ...
        else
        {
            ...
            res = frame2jpg_cb(fb, PORT, jpg_encode_stream, &jchunk) ? ESP_OK : ESP_FAIL;
    ...
    while (true)
    {
        ...
        else
        {
            ...
            if (!detection_enabled || fb->width > 400)
            {
                ...
                if (fb->format != PIXFORMAT_JPEG)
                {
                    bool jpeg_converted = frame2jpg(fb, PORT, &_jpg_buf, &_jpg_buf_len);
                    esp_camera_fb_return(fb);
                    fb = NULL;
                    if (!jpeg_converted)
                    {
            ...
            else
            {
                if (fb->format == PIXFORMAT_RGB565
                    ...
                    s = fmt2jpg(fb->buf, fb->len, fb->width, fb->height, PIXFORMAT_RGB565, PORT, &_jpg_buf, &_jpg_buf_len);
                    esp_camera_fb_return(fb);
                    fb = NULL;
                    if (!s) {
